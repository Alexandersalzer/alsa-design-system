...
export const Image: React.FC<ImageProps> = ({
  ...
  // Empty state — no src provided yet
  if (!resolvedSrc) {
    return (
      <Component componentKey={componentKey}>
        <div
          className={cn('image-container', `image-container--radius-${radius}`, 'image-container--empty', className)}
          style={{ ...containerStyles, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '6px', background: 'var(--surface-subtle)', border: '1px dashed var(--border-default)', color: 'var(--text-subtle)' }}
        >
          ...
          <span style={{ fontSize: '11px', fontWeight: 500, opacity: 0.6 }}>No image</span>
        </div>
      </Component>
    );
  }
